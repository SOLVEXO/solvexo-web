...
import { usePlatformImageEnhance } from '@/hooks/admin/useAdminAiStudio';
...
export function PlatformImageTool() {
  const [imageUrl, setImageUrl] = useState('');
  const [enhancementType, setEnhancementType] = useState<EnhancementType>('upscale');

  const { start, generating, error, result } = usePlatformImageEnhance();

  const handleGenerate = async () => {
    if (!imageUrl) return;
    await start({ imageUrl, enhancementType });
  };
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="bg-white border border-bone rounded-[10px] px-[22px] py-5">
        ...
        <div className="flex flex-col gap-4">
          <Field label="Banner / Marketing Image" hint="Upload a jpg, jpeg, png, or webp image">
            <ImageUpload value={imageUrl ? [imageUrl] : []} onChange={urls => setImageUrl(urls[0] ?? '')} />
          </Field>
      ...
      <div className="bg-white border border-bone rounded-[10px] px-[22px] py-5">
        ...
        {generating && (
          <div className="flex flex-col items-center justify-center py-[60px] text-center">
            ...
            <p className="text-sm font-semibold text-charcoal">Processing image…</p>
          </div>
        )}
        ...
        {result && result.status === 'succeeded' && (
          <div className="flex flex-col gap-4">
            <div>
              ...
              {result.originalImageUrl && <img src={result.originalImageUrl} alt="Original" className="w-full rounded-lg border border-bone" />}
            </div>
            <div>
              <p className="text-[10px] font-semibold text-slate uppercase tracking-[0.08em] mb-2">After</p>
              {result.enhancedImageUrl && <img src={result.enhancedImageUrl} alt="Enhanced" className="w-full rounded-lg border border-bone" />}
            </div>
