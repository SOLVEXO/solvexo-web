...
// to preserve.
type SettingSection = 'profile' | 'email-password' | 'share-story' | 'notifications' | 'delete-account';
...
const SETTINGS_NAV: { group: string; isDanger?: boolean; items: { id: SettingSection; label: string; Icon: LucideIcon }[] }[] = [
  {
    ...
    items: [
      ...
      { id: 'share-story',     label: 'Share Your Story', Icon: Quote          },
    ],
  },
  {
    group: 'Danger Zone',
...
export function SellerSettings({ variant = 'seller' }: { variant?: 'seller' | 'store' } = {}) {
  ...
  const validTabs: SettingSection[] = ['profile', 'email-password', 'share-story', 'notifications', 'delete-account'];
      ...
      <div className="px-4 lg:px-7 pt-5 pb-8">
        ...
        <div className={`${mobileDrilledIn ? 'grid' : 'hidden lg:grid'} grid-cols-1 lg:grid-cols-[1fr_260px] gap-5`}>
          ...
          <div>
            ...
            {/* Share Your Story section — same self-serve testimonial
               submission the dashboard's one-time popup also offers (see
               TestimonialPromptModal), always reachable here for a seller
               who dismissed that popup or wants to submit later. */}
            {active === 'share-story' && (
              <div className="bg-white border border-bone rounded-[10px] overflow-hidden">
            ...
            {/* Other sections */}
            {active !== 'profile' && active !== 'email-password' && active !== 'notifications' && active !== 'share-story' && (
              <div className="bg-white border border-bone rounded-[10px] px-4 sm:px-[26px] py-6">
