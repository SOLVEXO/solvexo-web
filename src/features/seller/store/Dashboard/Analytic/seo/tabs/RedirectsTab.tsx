...
export function RedirectsTab({ storeId }: RedirectsTabProps) {
  const [page, setPage] = useState(1);
  const { data, loading, error, refetch } = useSeoRedirects(storeId, { page, limit: 20 });
  ...
  const handleSave = async () => {
    const ok = editing === 'new'
      ? await createRedirect(storeId, { source, destination, statusCode })
      : await updateRedirect(storeId, (editing as RedirectRowData)._id, { source, destination, statusCode });
    if (ok) { setEditing(null); refetch(); }
  };

  const handleDelete = async (row: RedirectRowData) => {
    if (await deleteRedirect(storeId, row._id)) refetch();
  };

  const handleToggle = async (row: RedirectRowData, next: boolean) => {
    if (await updateRedirect(storeId, row._id, { isActive: next })) refetch();
  };
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card padding="none">
        ...
        {!loading && (data?.items ?? []).length === 0 && (
          <div className="px-5 py-10 text-center text-[12px] text-slate">No redirects configured yet.</div>
        )}
      </Card>
      ...
      {editing && (
        ...
        >
          <Field label="Source Path" className="mb-3">
            <Input value={source} onChange={e => setSource(e.target.value)} placeholder="/old-product-slug" />
          </Field>
          <Field label="Destination Path" className="mb-3">
            <Input value={destination} onChange={e => setDestination(e.target.value)} placeholder="/new-product-slug" />
          </Field>
