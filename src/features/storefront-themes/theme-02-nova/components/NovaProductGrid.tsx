 ...
 *  exactly mirroring `AtelierProductGrid`'s "one engine, three callers"
 *  shape with Nova's own independent presentation. */
  ...
  return (
    <div>
      <div className="flex items-end justify-between flex-wrap gap-3 mb-8">
        {heading && (
          <h1 style={{ fontFamily: t.fonts.display, fontSize: 'clamp(24px, 3vw, 34px)', fontWeight: 700, color: t.colors.ink }}>
            {heading}
          </h1>
        )}
        <div className="relative">
          <button
            ...
            style={{ fontFamily: t.fonts.body, fontSize: '12.5px', color: t.colors.ink, border: `1.5px solid ${t.colors.border}`, padding: '8px 14px', borderRadius: t.radius.sm }}
          >
            {SORT_OPTIONS.find(o => o.value === sort)?.label} <ChevronDown size={13} />
          </button>
          {sortOpen && (
            <div className="absolute right-0 z-10" style={{ top: 'calc(100% + 4px)', background: '#FFFFFF', border: `1.5px solid ${t.colors.border}`, minWidth: '190px', borderRadius: t.radius.sm, overflow: 'hidden' }}>
              {SORT_OPTIONS.map(o => (
                <button
                  key={o.value}
                  type="button"
      ...
      {products !== null && products.length === 0 && !error && (
        <div className="flex flex-col items-center text-center" style={{ padding: '80px 0', border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.md }}>
          <p style={{ fontFamily: t.fonts.display, fontSize: '18px', fontWeight: 700, color: t.colors.ink, marginBottom: '6px' }}>Nothing here yet</p>
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-3 mt-14">
              <NovaButton variant="outline" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>Previous</NovaButton>
              <span style={{ fontFamily: t.fonts.body, fontSize: '12.5px', color: t.colors.inkMuted }}>Page {page} of {totalPages}</span>
              <NovaButton variant="outline" disabled={page >= totalPages} onClick={() => setPage(p => p + 1)}>Next</NovaButton>
            </div>
          )}
        </>
      )}
