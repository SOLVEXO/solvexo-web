...
registerNovaSection('trust_badges', (_section: Section, blocks: Block[]) => {
  if (blocks.length === 0) return null;
  return (
    <div style={{ background: t.colors.bgAlt }}>
      <div
        ...
        {blocks.map((b, i) => {
          ...
          return (
            <div key={b._id ?? i} className="flex flex-col items-center gap-2">
              <div className="flex items-center justify-center" style={{ width: '38px', height: '38px', borderRadius: '9999px', background: t.colors.bg }}>
                <Icon size={17} style={{ color: t.colors.accent }} />
              </div>
              <p style={{ fontFamily: t.fonts.body, fontSize: '12px', color: t.colors.inkMuted, fontWeight: 600 }}>{b.settings.text}</p>
            </div>
          );
        })}
      </div>
