  ...
  return (
    <div style={{ background: t.colors.bgAlt, padding: `${t.layout.sectionPadY} ${t.layout.containerPadX}` }}>
      <div className="mx-auto flex flex-col items-center text-center gap-3" style={{ maxWidth: '480px' }}>
        <h2 style={{ fontFamily: t.fonts.display, fontSize: 'clamp(22px, 3vw, 32px)', fontWeight: 700, color: t.colors.ink }}>
          {section.settings.heading || 'Stay in the loop'}
        </h2>
        {section.settings.subtext && <p style={{ fontFamily: t.fonts.body, fontSize: '14px', color: t.colors.inkMuted }}>{section.settings.subtext}</p>}
        {status === 'done' ? (
          <p style={{ fontFamily: t.fonts.body, fontSize: '14px', color: t.colors.success, marginTop: '8px' }}>Thank you — you're subscribed.</p>
        ) : (
...
registerNovaSection('newsletter', (section: Section) => <NewsletterSection section={section} />);
