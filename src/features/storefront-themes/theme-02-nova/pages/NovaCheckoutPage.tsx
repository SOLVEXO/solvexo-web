...
function SectionCard({ step, title, done, children }: { step: number; title: string; done?: boolean; children: React.ReactNode }) {
  return (
    <div style={{ border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.md, overflow: 'hidden' }}>
      <div className="flex items-center gap-2.5" style={{ padding: '16px 20px', borderBottom: `1.5px solid ${t.colors.border}` }}>
        <span
          className="flex items-center justify-center shrink-0"
          style={{ width: '24px', height: '24px', borderRadius: '50%', background: done ? t.colors.success : t.colors.accent, color: t.colors.accentInk, fontSize: '11px', fontWeight: 700 }}
        >
          {done ? <CheckCircle2 size={13} /> : step}
        </span>
        <p style={{ fontFamily: t.fonts.display, fontSize: '15px', fontWeight: 700, color: t.colors.ink }}>{title}</p>
      </div>
...
/** Theme 02's own checkout — same real backend flow as `AtelierCheckoutPage`
 *  (address, shipping zones, coupon/gift-card, Stripe or Cash on Delivery,
 *  connected-gateway hand-off, payment-status polling) — the commerce logic
 *  is legitimate shared infra and is NOT reimplemented, only its Nova
 *  presentation (rounded cards, pill radio rows, accent-forward step badges). */
export function NovaCheckoutPage() {
  ...
  // wiring for the full doc comment.
  ...
  useEffect(() => {
    ...
    apiCreateCheckout({
      ...
      .then(async res => {
        ...
        // createCheckout always creates with a zero shipping fee — the real
        // fee (flat zone OR a real live carrier rate, re-verified server-
  ...
  /** Hands off to the gateway's own hosted checkout page (Safepay et al) —
   *  the buyer comes back to `NovaCheckoutReturnPage` (success) or straight
   *  back here (cancel); the real order is only ever created server-side
   *  once that gateway's webhook fires — nothing client-side here is
  ...
  return (
    <main className="mx-auto" style={{ maxWidth: t.layout.maxWidth, padding: `48px ${t.layout.containerPadX}` }}>
      <h1 style={{ fontFamily: t.fonts.display, fontSize: 'clamp(24px, 3vw, 32px)', fontWeight: 700, color: t.colors.ink, marginBottom: '28px' }}>Checkout</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8 items-start">
                ...
                <div className="flex flex-col gap-2.5">
                  {addresses.map(a => (
                    <label
                      ...
                      style={{ padding: '12px 14px', borderRadius: t.radius.sm, border: `1.5px solid ${selectedAddrId === a._id ? t.colors.accent : t.colors.border}` }}
                    >
                      <input
                      ...
                      <div style={{ fontFamily: t.fonts.body, fontSize: '12.5px' }}>
                        <p style={{ fontWeight: 700, color: t.colors.ink }}>{a.recipientName} · {a.phoneNumber}</p>
            ...
            {checkoutError ? (
              ...
              <div className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2.5">
                    {allowedMethods.map(m => (
                      <label
                        ...
                        style={{ padding: '12px 14px', borderRadius: t.radius.sm, border: `1.5px solid ${selectedMethod === m ? t.colors.accent : t.colors.border}` }}
                      >
                        ...
                        <span style={{ fontFamily: t.fonts.body, fontSize: '12.5px', fontWeight: 600, color: t.colors.ink }}>{m === 'stripe' ? 'Credit / Debit Card' : 'Cash on Delivery'}</span>
                      </label>
                    ))}
                    {extraMethods.map(m => (
                      <label
                        key={m.provider}
                        className="flex items-center gap-2.5 cursor-pointer"
                        style={{ padding: '12px 14px', borderRadius: t.radius.sm, border: `1.5px solid ${selectedMethod === m.provider ? t.colors.accent : t.colors.border}` }}
                      >
                        ...
                        <span style={{ fontFamily: t.fonts.body, fontSize: '12.5px', fontWeight: 600, color: t.colors.ink }}>{m.displayName}</span>
                      </label>
                    ))}
                  </div>
                )}
                ...
                {selectedMethod === 'stripe' && (
                  !isStripeConfigured() ? (
                    ...
                    <div className="flex flex-col items-center gap-2 text-center" style={{ padding: '24px 0' }}>
                      ...
                      <p style={{ fontFamily: t.fonts.body, fontSize: '13px', fontWeight: 600, color: t.colors.ink }}>Confirming your payment…</p>
                    </div>
                ...
                {selectedMethod === 'cash_on_delivery' && (
                  <NovaButton style={{ width: '100%', justifyContent: 'center' }} loading={placing} onClick={handlePlaceCod}>
                ...
                {extraMethods.some(m => m.provider === selectedMethod) && (() => {
                  ...
                  return (
                    <div className="flex flex-col gap-2.5">
                      ...
                      <NovaButton style={{ width: '100%', justifyContent: 'center' }} loading={initiatingExtra} onClick={() => handleInitiateExtraMethod(method.provider)}>
        ...
        <div className="flex flex-col gap-4" style={{ border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.md, padding: '22px' }}>
            ...
            <div className="flex flex-col gap-2.5 pt-1">
              {checkout.couponCode ? (
                ...
                <div className="flex gap-1.5">
                  ...
                  <button type="button" onClick={handleApplyCoupon} disabled={couponBusy || !couponInput.trim()} className="shrink-0 cursor-pointer bg-transparent" style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 700, padding: '0 14px', border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.sm, color: t.colors.ink }}>Apply</button>
                </div>
              )}
              ...
              {checkout.giftCardCode ? (
                ...
                <div className="flex gap-1.5">
                  ...
                  <button type="button" onClick={handleApplyGiftCard} disabled={giftCardBusy || !giftCardInput.trim()} className="shrink-0 cursor-pointer bg-transparent" style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 700, padding: '0 14px', border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.sm, color: t.colors.ink }}>Apply</button>
                </div>
              )}
          ...
          <div className="flex justify-between items-baseline">
            ...
            <span style={{ fontFamily: t.fonts.display, fontSize: '20px', fontWeight: 700, color: t.colors.ink }}>{symbol}{fmt2(total)}</span>
          </div>
