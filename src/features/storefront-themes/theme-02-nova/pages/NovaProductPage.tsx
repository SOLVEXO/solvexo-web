  ...
  return (
    <div className="flex flex-col gap-3 min-w-0">
      <div
        ...
        style={{ aspectRatio: '1/1', background: t.colors.bgAlt, borderRadius: t.radius.md, overflow: 'hidden' }}
      >
        ...
        <div className="flex gap-2 overflow-x-auto">
          {images.map((img, i) => (
            <button
              ...
              style={{ borderRadius: t.radius.sm, overflow: 'hidden', outline: selected === i ? `2px solid ${t.colors.accent}` : `1.5px solid ${t.colors.border}`, outlineOffset: '-1.5px' }}
            >
  ...
  return (
    <div className="flex flex-col gap-4 mb-6">
      {attributeNames.map(name => {
        ...
        return (
          <div key={name}>
            <p style={{ fontFamily: t.fonts.body, fontSize: '11.5px', fontWeight: 700, letterSpacing: '0.03em', textTransform: 'uppercase', color: t.colors.inkMuted, marginBottom: '8px' }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: active ? t.colors.accentInk : t.colors.ink,
                    }}
                  >
                    {val}
                  </button>
...
/** Theme 02's own Product page — same commerce-critical fixed core as
 *  `AtelierProductPage` (gallery, variant selection, quantity, add-to-cart,
 *  reviews stay outside the section system), with the surrounding content
 *  genuinely template-driven via the same `collection-template` backend
 *  infra. */
export function NovaProductPage() {
  ...
  if (error || !product) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 text-center" style={{ padding: `80px ${t.layout.containerPadX}` }}>
        ...
        <Link to="/" className="no-underline" style={{ fontFamily: t.fonts.body, fontSize: '12px', color: t.colors.accent, fontWeight: 600 }}>Back to shop</Link>
      </div>
    );
  }
        ...
        <div className="flex flex-col">
          ...
          {isDigital && product.digital?.previewAvailable && (
            <button
              ...
              style={{ fontFamily: t.fonts.body, fontSize: '13px', fontWeight: 700, color: t.colors.ink, border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.sm, padding: '11px 0', marginBottom: '12px' }}
            >
