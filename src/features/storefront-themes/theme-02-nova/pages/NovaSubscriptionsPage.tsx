...
/** Theme 02's own Subscriptions (seller membership plans) page — ported
 *  functionally 1:1 from `AtelierSubscriptionsPage`, restyled with Nova's
 *  rounded/pill vocabulary. */
export function NovaSubscriptionsPage() {
  ...
  return (
    <main className="mx-auto" style={{ maxWidth: '760px', padding: `48px ${t.layout.containerPadX}` }}>
      <h1 style={{ fontFamily: t.fonts.display, fontSize: '24px', fontWeight: 700, color: t.colors.ink, marginBottom: '8px' }}>Membership</h1>
      ...
      {loading ? (
        ...
        <section style={{ border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.md, padding: '24px' }}>
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p style={{ fontFamily: t.fonts.display, fontSize: '18px', fontWeight: 700, color: t.colors.ink, marginTop: '4px', textTransform: 'capitalize' }}>{mySub.status}</p>
            </div>
          ...
          <div className="flex items-center gap-3" style={{ marginTop: '18px' }}>
            {!mySub.pendingCancellation && (
              <button
                ...
                style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 700, color: t.colors.ink, border: `1.5px solid ${t.colors.border}`, borderRadius: '9999px', padding: '8px 16px' }}
              >
            ...
            {!mySub.pendingCancellation && mySub.status !== 'canceled' && (
              <button
                ...
                className="cursor-pointer bg-transparent disabled:opacity-50"
                style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 700, color: t.colors.danger, border: `1.5px solid ${t.colors.danger}`, borderRadius: '9999px', padding: '8px 16px' }}
              >
                Cancel
              </button>
            )}
        ...
        <div className="flex flex-col items-center text-center" style={{ padding: '64px 0', border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.md }}>
          ...
          <p style={{ fontFamily: t.fonts.display, fontSize: '16px', fontWeight: 700, color: t.colors.ink }}>No membership plans yet</p>
        ...
        <>
          <div className="flex items-center gap-1" style={{ marginBottom: '20px' }}>
            {(['monthly', 'yearly'] as const).map(iv => (
              <button
                ...
                style={{
                  fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 700, padding: '8px 16px', borderRadius: '9999px',
                  border: `1.5px solid ${interval === iv ? t.colors.accent : t.colors.border}`,
                  background: interval === iv ? t.colors.accent : 'transparent',
          ...
          <div className="flex flex-col gap-4">
            {plans.map(plan => {
              ...
              return (
                <div key={plan._id} style={{ border: `1.5px solid ${t.colors.border}`, borderRadius: t.radius.md, padding: '22px' }}>
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                      <p style={{ fontFamily: t.fonts.display, fontSize: '17px', fontWeight: 700, color: t.colors.ink }}>{plan.name}</p>
                    ...
                    <p style={{ fontFamily: t.fonts.display, fontSize: '20px', fontWeight: 700, color: t.colors.ink }}>
                  ...
                  <div style={{ marginTop: '18px' }}>
                    <NovaButton onClick={() => handleSubscribe(plan)} loading={isSubscribing}>Join {plan.name}</NovaButton>
                  </div>
                </div>
              );
            })}
