...
/** Theme 01's own Wishlist page — real, backend-wired (`useWishlistContext`,
 *  the same context the product-card heart icon already writes to; this
 *  page is just the missing "view the full list" surface). Deliberately a
  ...
  return (
    <main className="mx-auto" style={{ maxWidth: '960px', padding: `48px ${t.layout.containerPadX}` }}>
      <div style={{ marginBottom: '28px' }}>
        <h1 style={{ fontFamily: t.fonts.display, fontSize: '24px', fontWeight: 600, color: t.colors.ink }}>My Wishlist</h1>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="animate-pulse flex gap-4" style={{ border: `1px solid ${t.colors.border}`, padding: '16px' }}>
              <div style={{ width: '84px', height: '84px', background: t.colors.bgAlt, flexShrink: 0 }} />
              <div className="flex-1 flex flex-col gap-2 justify-center">
                <div style={{ height: '12px', width: '70%', background: t.colors.bgAlt }} />
                <div style={{ height: '12px', width: '40%', background: t.colors.bgAlt }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center text-center" style={{ padding: '64px 0', border: `1px solid ${t.colors.border}` }}>
          ...
          <p style={{ fontFamily: t.fonts.display, fontSize: '16px', fontWeight: 600, color: t.colors.ink }}>Your wishlist is empty</p>
          <p style={{ fontFamily: t.fonts.body, fontSize: '13px', color: t.colors.inkMuted, marginTop: '6px', marginBottom: '20px' }}>
            Tap the heart on any product to save it here.
          </p>
          <AtelierButton onClick={() => { window.location.href = '/'; }}>Continue Shopping</AtelierButton>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {wishlistItems.map(({ product, variants }) => {
            ...
            return (
              <div key={product._id} className="flex gap-4" style={{ border: `1px solid ${t.colors.border}`, padding: '16px' }}>
                <Link to={`/product/${product.slug}`} style={{ width: '84px', height: '84px', flexShrink: 0, background: t.colors.bgAlt, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  {product.images?.[0]
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div>
                    <Link to={`/product/${product.slug}`} className="no-underline" style={{ fontFamily: t.fonts.display, fontSize: '14px', fontWeight: 600, color: t.colors.ink }}>
                      {product.name}
                    </Link>
