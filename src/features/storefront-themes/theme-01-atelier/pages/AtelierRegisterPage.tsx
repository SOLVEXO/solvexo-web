  ...
  return (
    <div className="mx-auto" style={{ maxWidth: '400px', padding: '72px 20px' }}>
      <div className="text-center" style={{ marginBottom: '32px' }}>
        <h1 style={{ fontFamily: t.fonts.display, fontSize: '24px', fontWeight: 600, color: t.colors.ink }}>Create your {store.name} account</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 atelier-form" style={{ border: `1px solid ${t.colors.border}`, padding: '28px' }}>
        <div>
          <label htmlFor="atelier-register-name" style={atelierLabel}>Full Name</label>
          <input id="atelier-register-name" required autoComplete="name" value={name} onChange={e => setName(e.target.value)} style={atelierInput} />
        </div>
        <div>
          <label htmlFor="atelier-register-email" style={atelierLabel}>Email</label>
        ...
        <div>
          ...
          <input id="atelier-register-address" required autoComplete="street-address" value={address} onChange={e => setAddress(e.target.value)} style={atelierInput} />
        </div>
        <div>
          <label htmlFor="atelier-register-password" style={atelierLabel}>Password</label>
          <div className="relative">
            <input
              id="atelier-register-password"
        ...
        <AtelierButton type="submit" style={{ width: '100%', justifyContent: 'center', marginTop: '8px' }} loading={register.loading}>
          Create Account
        </AtelierButton>
      </form>
      ...
      <p className="text-center" style={{ fontFamily: t.fonts.body, fontSize: '13px', color: t.colors.inkMuted, marginTop: '18px' }}>
        ...
        <a href={`login${redirectTo !== '/' ? `?redirect=${encodeURIComponent(redirectTo)}` : ''}`} style={{ color: t.colors.accent, fontWeight: 600, textDecoration: 'none' }}>
          Sign in
        </a>
      </p>
    </div>
