...
/** Theme 01's own Gift Card purchase page — real, backend-wired
 *  (`apiGetGiftCardPublicSettings`/`apiCreateGiftCardPurchaseIntent`, the
 *  same Stripe PaymentIntent flow the old apex `SellerStorefront.tsx` denom-
 *  picker modal used, reusing `StripeCardPayment` as-is). The real `GiftCard`
...
export function AtelierGiftCardsPage() {
  ...
  return (
    <main className="mx-auto" style={{ maxWidth: '520px', padding: `48px ${t.layout.containerPadX}` }}>
      <h1 style={{ fontFamily: t.fonts.display, fontSize: '24px', fontWeight: 600, color: t.colors.ink, marginBottom: '8px' }}>Gift Cards</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center text-center" style={{ padding: '64px 0', border: `1px solid ${t.colors.border}` }}>
          ...
          <p style={{ fontFamily: t.fonts.display, fontSize: '16px', fontWeight: 600, color: t.colors.ink }}>Gift cards aren't available</p>
        ...
        <div className="flex flex-col items-center text-center" style={{ padding: '64px 0', border: `1px solid ${t.colors.border}` }}>
          ...
          <p style={{ fontFamily: t.fonts.display, fontSize: '16px', fontWeight: 600, color: t.colors.ink }}>Payment received</p>
        ...
        <>
          <p style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 600, color: t.colors.ink, marginBottom: '10px' }}>Choose an amount</p>
          <div className="grid grid-cols-3 gap-2.5" style={{ marginBottom: '22px' }}>
            {settings.denominations.map(d => (
              <button
                ...
                style={{
                  fontFamily: t.fonts.body, fontSize: '14px', fontWeight: 600, padding: '14px 0',
                  border: `1px solid ${amount === d ? t.colors.ink : t.colors.border}`,
                  background: amount === d ? t.colors.ink : 'transparent',
          ...
          <label htmlFor="atelier-gc-email" style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 600, color: t.colors.ink, display: 'block', marginBottom: '5px' }}>Recipient email</label>
          <input
            id="atelier-gc-email" type="email" value={recipientEmail} onChange={e => setRecipientEmail(e.target.value)}
            className="w-full" style={{ fontFamily: t.fonts.body, fontSize: '13px', border: `1px solid ${t.colors.border}`, padding: '10px 12px', marginBottom: '14px', color: t.colors.ink }}
          />
          <label htmlFor="atelier-gc-name" style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 600, color: t.colors.ink, display: 'block', marginBottom: '5px' }}>Recipient name <span style={{ color: t.colors.inkMuted, fontWeight: 400 }}>(optional)</span></label>
          <input
            id="atelier-gc-name" value={recipientName} onChange={e => setRecipientName(e.target.value)}
            className="w-full" style={{ fontFamily: t.fonts.body, fontSize: '13px', border: `1px solid ${t.colors.border}`, padding: '10px 12px', marginBottom: '14px', color: t.colors.ink }}
          />
          <label htmlFor="atelier-gc-msg" style={{ fontFamily: t.fonts.body, fontSize: '12px', fontWeight: 600, color: t.colors.ink, display: 'block', marginBottom: '5px' }}>Message <span style={{ color: t.colors.inkMuted, fontWeight: 400 }}>(optional)</span></label>
          <textarea
            id="atelier-gc-msg" value={message} onChange={e => setMessage(e.target.value)} rows={3}
            className="w-full" style={{ fontFamily: t.fonts.body, fontSize: '13px', border: `1px solid ${t.colors.border}`, padding: '10px 12px', marginBottom: '18px', resize: 'vertical', color: t.colors.ink }}
          />
          ...
          <AtelierButton onClick={handleContinue} loading={creating}>Continue to Payment</AtelierButton>
        </>
      )}
    </main>
  );
