 ...
 *  (Safepay et al — see `CheckoutPaymentMethodsService`/`useWhatsAppEmbeddedSignup`'s
 *  sibling doc comments for the rest of this flow). The gateway's own
 *  webhook is what actually finalizes the order server-side
 *  (`PaymentService.finalizeGatewayPayment`) — this page never trusts
 *  anything in its own URL/query string as proof of payment, it only polls
 *  the same generic, provider-agnostic `apiGetPaymentStatus` the existing
 *  Stripe flow already uses (see `AtelierCheckoutPage.handleStripeConfirmed`)
 *  until that status flips, exactly the same "don't trust the client,
 *  trust the webhook" posture as the rest of checkout. */
export function AtelierCheckoutReturnPage() {
  ...
  if (status === 'failed') {
    return (
      <div className="mx-auto text-center" style={{ maxWidth: '480px', padding: '72px 20px' }}>
        <div className="flex items-center justify-center mx-auto" style={{ width: '56px', height: '56px', borderRadius: '50%', background: '#FBEAEA', marginBottom: '20px' }}>
          <XCircle size={28} style={{ color: t.colors.danger }} />
        </div>
        <h1 style={{ fontFamily: t.fonts.display, fontSize: '22px', fontWeight: 600, color: t.colors.ink, marginBottom: '8px' }}>Payment wasn't completed</h1>
        ...
        <AtelierButton onClick={() => navigate('/checkout')}>Back to Checkout</AtelierButton>
      </div>
    );
  }
...
export default AtelierCheckoutReturnPage;
