...
export function Homepage() {
  ...
  return (
    <div className="bg-white min-h-full">
      ...
      {/* ── Hero ─────────────────────────────────────────────────────────────── */}
      {/* px-12 matches the nav bar padding exactly */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[#141413] to-[#2C2A28] px-12 py-20">

        {/* Decorative circles — right side of hero */}
        <div className="absolute rounded-full w-[420px] h-[420px] bg-[#3A3633] -top-[80px] -right-[80px]" />
        <div className="absolute rounded-full w-[260px] h-[260px] bg-[#2E2B28] bottom-[-50px] right-[28%]" />

        {/* Content — constrained to left column */}
        <div className="relative z-[1] max-w-[640px]">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 rounded-full px-[14px] py-[5px] mb-6 border border-[rgba(217,119,87,0.35)] bg-[rgba(217,119,87,0.12)]">
            <Sparkles size={12} className="text-brand-orange" />
            <span className="text-[12px] font-medium text-brand-orange">AI-powered commerce. One platform.</span>
          </div>

          {/* Heading */}
          <h1 className="text-[48px] font-semibold text-white leading-[1.15] mb-5 font-serif">
            The Commerce OS for Sellers, Creators &amp; Educators
          </h1>

          {/* Subtext */}
          <p className="text-[15px] text-[#B0AEA8] leading-[1.7] mb-9 max-w-[500px]">
            Sell physical products, digital downloads, and educational resources — with AI-powered tools,
            a built-in marketplace, and point-of-sale. Everything commerce, in one place.
          </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-[13px] rounded-lg text-[15px] font-medium text-white border border-[rgba(255,255,255,0.25)] bg-transparent hover:bg-[rgba(255,255,255,0.08)] transition-colors cursor-pointer"
            >
              Browse Marketplace
            </button>
          </div>
          ...
          <div className="flex gap-8 mt-10">
            {STATS.map(({ value, label }) => (
              <div key={label}>
                <p className="text-[20px] font-bold text-brand-orange">{value}</p>
                <p className="text-[12px] text-[#8C8A82] mt-[2px]">{label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-12 py-16">
        <p className="text-[12px] font-semibold text-brand-orange text-center uppercase tracking-[0.1em] mb-3">
          Built for every type of seller
        </p>
        <h2 className="text-[32px] font-bold text-carbon text-center mb-12 font-serif">
          ...
          {FEATURES.map(f => (
            <Card key={f.title} hover padding="none" onClick={() => navigate(f.path)}>
              <div className="p-6">
                ...
                <p className="text-[16px] font-bold text-[#141413] mb-2">{f.title}</p>
                <p className="text-[13px] text-[#8C8A82] leading-[1.6] mb-[14px]">{f.desc}</p>
                <span className="inline-flex items-center gap-1 text-[12px] font-semibold text-brand-orange cursor-pointer">
      ...
      {/* ── Social Proof ───────────────────────────────────────────────────────── */}
      <div className="bg-[#FAF9F5] border-t border-bone px-12 py-12">
        <p className="text-[12px] font-semibold text-[#8C8A82] text-center uppercase tracking-[0.08em] mb-8">
        ...
        <div className="grid grid-cols-3 gap-5">
          {TESTIMONIALS.map(t => (
            <Card key={t.name} padding="none">
              <div className="p-6">
                <p className="text-[13px] text-charcoal leading-[1.7] mb-4 italic">"{t.text}"</p>
                <div className="flex items-center gap-[10px]">
                  <Avatar name={t.name} size={32} />
                  <div>
                    <p className="text-[13px] font-semibold text-[#141413]">{t.name}</p>
      ...
      <div className="bg-brand-orange px-12 py-[60px] text-center">
        <h2 className="text-[32px] font-bold text-white mb-[14px] font-serif">
          Ready to start selling?
        </h2>
        <p className="text-[15px] text-[rgba(255,255,255,0.85)] mb-7">
