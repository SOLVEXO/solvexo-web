...
function TypingBubble({ name, image }: { name: string; image?: string | null }) {
  return (
    <div className="flex items-end gap-[6px] mb-[6px] transition-all duration-300 ease-out starting:opacity-0 starting:translate-y-1">
      ...
      <div className="bg-white border border-[#EEECE4] rounded-[18px] rounded-bl-[4px] px-[16px] py-[12px] shadow-[0_1px_2px_rgba(0,0,0,0.07)]">
    ...
    <div className="relative flex-1 min-h-0">
      ...
      {showJumpToLatest && (
        <button
          ...
          className="absolute bottom-3 right-3 w-9 h-9 rounded-full bg-white border border-bone shadow-[0_4px_12px_rgba(0,0,0,0.12)] flex items-center justify-center cursor-pointer text-charcoal hover:bg-cream transition-colors"
        >
          ↓
        </button>
      )}
